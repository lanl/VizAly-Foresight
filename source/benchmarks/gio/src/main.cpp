...
int main(int argc, char **argv)
{
	...
	std::size_t num_buckets = std::stoul(std::string(argv[5]));
	std::size_t blocksize = std::stoul(std::string(argv[6]));
	...
	cbench.data = &data;
	cbench.cdata = &cdata;
	cbench.num_buckets = num_buckets;
	cbench.blocksize = blocksize;
	...
	dbench.data = &udata;
	dbench.cdata = &cdata;
	dbench.num_buckets = num_buckets;
	dbench.blocksize = blocksize;
	...
	if(rank == 0)
	{
		...
		std::stringstream ss;
		ss << in_name << ";" << format << ";" << repetitions << ";" << size << ";" << num_buckets << ";" << blocksize << ";";
