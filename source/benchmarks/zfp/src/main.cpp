...
int main(int argc, char **argv)
{
	...
	int mode = std::stoi(std::string(argv[6]));
	std::size_t param = std::stoul(std::string(argv[7]));
	std::size_t nthreads = std::stoul(std::string(argv[8]));
	...
	cbench.data = &data;
	cbench.cdata = &cdata;
	cbench.mode = static_cast<compression_benchmark::mode_t>(mode);
	cbench.param = param;
	...
	dbench.data = &udata;
	dbench.cdata = &cdata;
	dbench.mode = static_cast<decompression_benchmark::mode_t>(mode);
	dbench.param = param;
	...
	if(rank == 0)
	{
		...
		std::stringstream ss;
		ss << in_name << ";" << format << ";" << repetitions << ";" << size << ";" << mode << ";" << param << ";" << nthreads << ";";
